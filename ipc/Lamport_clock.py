...
class TimestampE:
        def __init__( self,event_t, data,tist, id):
        ...
        def condic( self):
            return{"event_t":self.event_t, "data":self.data, "lamport_time": self.tist, "id": self.id}
        
        @classmethod
        def  fr_dic(cls,  d):
            return cls(event_t= d["event_type"], data= d["data"], 
                       timestamp =d["lamport_time"], id =d["id"] )
